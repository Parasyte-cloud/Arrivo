...
| Login / accounts | ✅ Real — full signup/login backed by `arrivo-backend`, JWT sessions, token stored securely on-device |
| Payments | ✅ Real — Paystack checkout, verified server-side, tested end-to-end with a real test transaction |
| Driver verification / ID display | ✅ Real data model — driver verification is enforced server-side, but this screen still shows a static demo driver card rather than the real assigned driver's details (a small remaining wiring task, not a missing feature) |
| Booking types (one-way / full day / full week / full month) | ✅ Real — selectable on the Route screen, saved to the ride record, tested against a real database |
| **Rider panic button** | ✅ Added this update — a safety button on the Live Tracking screen posts to the backend immediately and surfaces in the admin dashboard's Panic Alerts page within its 10-second poll. **Important honest gap**: this currently only notifies whoever has the admin dashboard open — it does not yet send an SMS or push alert to a human. See `arrivo-backend`'s README for what's needed before relying on this for real safety incidents. |
| Live driver GPS location | 🟡 The *data* is now real — the driver app reports GPS every 20 seconds, and `GET /api/rides/:id` returns it. What's still a placeholder is the *map visualization* itself — swap `MapPlaceholder` for `react-native-maps` to actually plot the coordinates that are already flowing in. |
