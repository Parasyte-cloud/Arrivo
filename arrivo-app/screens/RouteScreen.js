  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 40 }}>
        ...
        <Card style={{ marginTop: spacing.md }}>
          ...
          {VEHICLES.map((v) => (
            <Pressable key={v.id} onPress={() => setVehicle(v.id)} style={styles.vehicleRow}>
              ...
              <Text style={styles.vehiclePrice}>₦{v.price.toLocaleString()}</Text>
            </Pressable>
          ))}
        </Card>
        ...
        <Button
          label={`Confirm Route · ₦${selected.price.toLocaleString()}`}
          onPress={() =>
            navigation.navigate("Checkout", {
              amountNaira: selected.price,
              label: `${selected.label} — Airport Pickup`,
